...
The frontend loads market data once, then refreshes after confirmed writes or **Refresh market**. It does not poll market data every 30 seconds. A successful intent commit queues exactly one search of **all live Event 1 intents** through `/api/solve/pool`, after its receipt block is indexed. An indexing failure retains that queued search until a user retries indexing successfully. Cancelling approval or signing never queues a search. Further searches require **Check all intents** or **Run solver**; no-match results, search failures, wallet changes, page navigation and market refreshes do not trigger matching. The backend discovers the pool independently of the viewing wallet or selected checkboxes, verifies committed hashes, re-reads registry state, custody and payment capacity, and searches combinations of two to four participants. Four is the maximum size of a candidate settlement, not the size of the input pool.

Matching is bounded to 100 candidates and an eight-second computation budget, with at most 100 ticket assignments per participant subset. Agent what-if searches retain their two-second per-call limit. The solver caches intent hashes and ticket eligibility within one search, removes requests whose custody or available supply makes them impossible, rejects overlapping offered NFTs before allocation, and assigns scarce ticket choices first. These checks use the same snapshot without additional Graph queries. Every constructed candidate still passes full validation before ranking and simulation.

Evidence records the actual bounds, termination, request combinations checked and counts for every exclusion reason. It retains the first 2,000 discarded-subset details and every constructed candidate rejection; omitted subset details are explicitly counted. A complete bounded search is not a claim that every possible assignment was enumerated. A timeout without a candidate displays **Search paused**, separately from a completed search without a match. Reaching the candidate cap does not disable a simulated match. Intents with more than four offered/received tickets are explicitly excluded with a reason. Above 256 live intents the service returns an explicit capacity error, rather than silently searching a prefix. Concurrent identical pool requests share in-flight work; each later Graph search reads the current pool. Simulation does not reserve state.

The frontend does not send settlement transactions automatically. A proposer uses **Propose and settle**, which simulates exactly the reviewed candidate against current chain state before submitting and paying gas. A simulation rejection clears the candidate and invites another manual search. There is no continuously running background settlement worker in this UI.

The intent pool shows other participants' offers and signed conditions. **Check all intents** runs one search of the latest pool, even when the displayed snapshot has fewer than two requests. Selecting checkboxes switches the search scope to two to four requests, which **Run solver** sends through `/api/solve`. **Check all intents** also returns from selected requests to the whole pool. Ordinary users do not need to choose counterparties before submitting an intent. The workspace navigation always exposes **Matching**, including after reloading an existing request.

The **Intent pool (N)** button opens the request list in a native modal dialog. The list starts closed and uses already-loaded public state. Opening or closing it does not start a query or search. Escape and Close dismiss it, restore focus to the opener and restore background scrolling. Manual selection, search and revocation remain available inside it. Matching results and **Propose and settle** stay on the main page. **Past settlements** shows readable swap labels; opening a receipt reveals its real transaction hash. Technical search evidence and ranking details are disclosures.

Rechecking keeps the previous candidate visible and labels it as being rechecked, with settlement disabled until the new result arrives. A failed search or a no-match result clears the obsolete candidate. The open/closed choices for search evidence and match explanation persist across manual checks and transaction read-backs, including when results temporarily disappear. Refreshing results never closes the intent-pool dialog.
...
The persistent **Your latest swap request** panel distinguishes a request ready to check, searching, waiting for a match, a simulated match awaiting settlement, solver outage, revocation, expiry and confirmed settlement. A candidate must include the exact request hash and pass simulation before it is called a match; only registry state `SETTLED` produces **Swap confirmed**. Existing transaction receipt panels continue to require a successful receipt. Waiting is not a claim that no possible match exists. Reopening the page with the same wallet restores the latest request from public chain state without starting a search. The on-chain intent remains available to other proposers while the user is away.

`node scripts/test-matching-status.mjs` checks request selection and status rules. The mock-wallet browser suite covers idle pages without periodic reads, exactly one search after commit, indexing recovery before that search, explicit retries, manual selection, no-match results, failed simulation and terminal chain states without broadcasting real transactions. Initial event loading retains its retry loop and provider cooldown; transaction read-backs still wait for the confirmed block to be indexed.
