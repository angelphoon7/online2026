...
export default function IntentBuilder({ market, account, approved, busy, seatMapOpen, onSeatMapClose, onComplete, onApprove, onCustody, onDepositSelected, onSign, onDemo, onConnect, connectionError }: {
  ...
  onDemo: () => Promise<unknown>;
  onCustody: (ticket: ChainTicket, mode: 'deposit' | 'withdraw') => Promise<unknown>;
  onDepositSelected: (tokenIds: bigint[]) => Promise<unknown>;
  onSign: (intent: IntentParams, prepared: (intent: IntentParams) => void) => Promise<boolean>;
}) {
      ...
      return <section key={title} className={`workspace-panel intent-step ${expanded ? 'is-expanded' : visited ? 'is-complete' : 'is-future'}`} data-step={number} data-expanded={expanded}>
        ...
        {expanded && <div className="step-content" aria-labelledby={`step-title-${number}`}>
          {number === 2 && <>
            ...
            <button className="primary full sign-intent" disabled={busy || !account || !intent.offered.length || toDeposit.length > 0 || choiceInvalid || timingUnavailable} onClick={() => void onSign(intent, setPrepared).then(completed => { if (completed) onComplete(); })}>Sign and commit <span>↗</span></button>
