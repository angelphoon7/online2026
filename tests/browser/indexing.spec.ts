...
test('receipt floor survives errors, stale responses, agent hash changes and reload', async ({ page }) => {
  ...
  await page.locator('.activity-toast-open').click();
  const activity = page.getByRole('dialog', { name: 'Details', exact: true });
  await expect(activity.locator(`a[href$="${REVOKE}"]`)).toBeVisible();
  await expect(activity.locator(`a[href$="${COMMIT}"]`)).toBeVisible();
  await activity.getByRole('button', { name: 'Close details', exact: true }).click();
...
test('partial budget failure retains revoke receipt and waits before showing the pool', async ({ page }) => {
  ...
  await expect(page.locator('.activity-toast-copy strong')).toHaveText('Revoke confirmed; replacement was not confirmed.');
  await page.getByRole('button', { name: 'Revoke confirmed; replacement was not confirmed. View details', exact: true }).click();
  const activity = page.getByRole('dialog', { name: 'Details', exact: true });
  await expect(activity.getByText('Revoke confirmed; replacement was not confirmed.', { exact: true })).toHaveCount(1);
  await expect(activity.locator(`a[href$="${REVOKE}"]`)).toBeVisible();
  await activity.getByRole('button', { name: 'Close details', exact: true }).click();
