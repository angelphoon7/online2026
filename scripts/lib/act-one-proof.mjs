...
export function inspectActOneReceipt(deployment, evidence, transaction, receipt, block) {
  ...
  assert.equal(new Set(intents.map(i => lower(i.owner))).size, 3);
  ...
  assert.equal(settled[0].args.participantCount, 3n);
...
export async function auditActOne(client, deployment, evidence, hash) {
  ...
  const proof = inspectActOneReceipt(deployment, evidence, transaction, receipt, block);
