...
assert.deepEqual(demoPriceQuote({ ...draft, sectionMask: 2n }, pricedTickets), { offeredTotal: 2000000n, wantedTotal: 2020000n, paymentAmount: 20000n });
assert.equal(demoPriceQuote({ ...draft, sectionMask: 1n }, pricedTickets).paymentAmount, 0n);
assert.equal(demoPriceQuote({ ...draft, sectionMask: 1n }, pricedTickets.map(t => ({ ...t, sectionId: 1 }))).paymentAmount, -20000n);
assert.equal(demoPriceQuote({ ...draft, sectionMask: 2n, exactCount: 1 }, pricedTickets).paymentAmount, -990000n);
assert.equal(demoPriceQuote({ ...draft, sectionMask: 3n }, pricedTickets), null);
assert.equal(demoPriceQuote({ ...draft, sectionMask: 4n }, pricedTickets).wantedTotal, 2040000n);
assert.equal(demoPriceQuote({ ...draft, sectionMask: 8n }, pricedTickets).wantedTotal, 2060000n);
