...
try {
  ...
  await call('Page.navigate',{url:base+'/?preloadCheck=1#events'});
  await waitFor(`window.testWallet?.marketReads === 1 && !!document.querySelector('.poster-live')`);
  if(await evaluate(`document.querySelector('.poster-live').disabled`))throw Error('Poster must remain clickable while public data loads');
  ...
  await click('AFTER');
  await waitFor(`location.hash === '#workspace' && [...document.querySelectorAll('dialog[open]')].some(d => d.textContent.includes('Loading…'))`);
  ...
  await waitFor(`!!document.getElementById('workspace') && !document.querySelector('dialog[open]')`);
  console.log('PASS poster opens during public reads, shows a loading dialog, and reveals the workspace when ready without another fetch.');
  ...
  const reviewReads=await evaluate('window.testWallet.marketReads');
  const reviewSearches=await evaluate('window.testWallet.poolCalls');
  await evaluate('window.testWallet.poll()');
  await waitFor(`window.testWallet.marketReads>${reviewReads}`);
  ...
  await assert(`window.testWallet.poolCalls===${reviewSearches} && !Array.from(document.querySelectorAll('button')).find(b=>b.textContent.includes('Propose and settle')).disabled`, 'Background read restarted the solver or blocked the ready match');
  ...
  await evaluate('window.testWallet.noMatch=true');await click('Check all intents');
  ...
  console.log('PASS explicit searches preserve open details; background reads keep the reviewed match available.');
  ...
  await navigate();await offer();await evaluate('window.testWallet.cancel=true');await evaluate("document.querySelector('.sign-intent').click()");
  ...
  await assert("!document.querySelector('.budget-slider') && !!document.querySelector('.quote-total')", 'Mobile payment must show the fixed difference');
  ...
  await evaluate('window.testWallet.unrelated=false;window.testWallet.badSimulation=true');await click('Check all intents');
  ...
  await evaluate("document.querySelector('.sign-intent').click()");
  ...
  await evaluate('window.testWallet.budgetReached=true');await click('Check all intents');
  await waitFor(`document.body.innerText.includes('Search budget reached')`);
  const searchesBeforeSettle=await evaluate('window.testWallet.solveCalls.length');
  await click('Propose and settle');
  await waitFor(`window.testWallet.calls.includes('eth_sendTransaction')`);
  await assert(`window.testWallet.solveCalls.length===${searchesBeforeSettle}`, 'Settlement replaced the reviewed candidate with a new solver search');
  ...
  console.log('PASS six-intent pool automatically searched, budget reported, reviewed three-intent settlement submitted without another search, and remaining pool continues after personal settlement.');
  ...
  const claimReplacement = async () => {
    await waitFor(`document.querySelector('.swap-nft-import button') && !document.querySelector('.swap-nft-import button').disabled`);
    await click('Claim my tickets');
  };
  await openSettlement();await click('Propose and settle');
  await waitFor(`document.querySelector('.swap-nft-import')`);
  await assert("!window.testWallet.calls.includes('wallet_watchAsset')", 'Settlement opened NFT imports before Claim was clicked');
  ...
  await assert("document.querySelector('.receipt-section').textContent.includes('Settlement confirmed') && document.querySelector('.swap-nft-import').textContent.includes('Import NFT')", 'Unsupported wallet hid successful swap/manual import details');
  ...
  await assert("!document.querySelector('.swap-nft-import') && !window.testWallet.calls.includes('wallet_watchAsset')", 'Independent proposer was shown a claim for another participant tickets');
