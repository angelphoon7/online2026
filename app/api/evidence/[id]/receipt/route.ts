...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json(saveEvidence({ ...record, transactionHash: hash, receipt: { status: receipt.status, blockNumber: receipt.blockNumber.toString(), blockHash: receipt.blockHash, participantCount: event.participantCount.toString(), confirmed: true } }, id));
  } catch {
