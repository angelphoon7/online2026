...
import { consumeChallenge, readChallenge, saveChallenge } from '@/server/claim-challenges';
import { consumeQuota } from '@/server/durable-store';
...
function failure(error: unknown) {
  return Response.json({ error: error instanceof DemoTicketError || error instanceof SigningBusy ? error.message : 'Demo ticket issuance could not finish. Retry Get free tickets to resume your claim; the issuer must have test USDC for gas.' }, { status: error instanceof DemoTicketError || error instanceof SigningBusy ? error.status : 503, headers });
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!await consumeQuota('claim-challenges', 100, 60_000)) throw new DemoTicketError('Too many pending demo claims. Retry shortly.', 429);
    ...
    await saveChallenge(message, { recipient, expires });
...
export async function POST(request: Request) {
  try {
    ...
    const claim = await readChallenge(message);
    if (!claim) throw new DemoTicketError('Demo claim expired. Click Get free tickets again.', 401);
    if (!await verifyMessage({ address: claim.value.recipient, message, signature })) throw new DemoTicketError('The claim must be signed by the receiving wallet.', 401);
    ...
    if (!await consumeChallenge(message, claim.raw)) throw new DemoTicketError('This demo claim is already being processed.', 409);
    return Response.json(await issueDemoTickets(claim.value.recipient), { headers });
  } catch (error) { return failure(error); }
}
